...
export const FinancialStatementTable: React.FC<FinancialStatementTableProps> = ({ data }) => {
  ...
  const months = data.map((d) => {
    if (typeof d.date === 'string' || typeof d.date === 'number') {
      return new Date(d.date).toLocaleString('default', { month: 'short' });
    }
    return d.date.toLocaleString('default', { month: 'short' });
  });

  const getMonthlyValues = (selector: (d: MonthlyFinancials) => number): number[] => {
    return data.map(selector);
  };

  return (
    <Box bg="white" borderRadius="lg" boxShadow="sm" position="relative">
